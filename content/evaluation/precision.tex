...
While changing from \SI{200}{\milli\second} to \SI{100}{\milli\second} almost increased accuracy by \perc{100}\todo[author=Kasper]{It used to state that the accuracy was increased by \perc{200} but I think that that is wrong. $\frac{281-141}{281}*100 = 49.82$ so roughly half. Half the error must mean double the accuracy, but that means an increase of \perc{100} and not \perc{200}. \perc{200} would be triple the accuracy and that can't be the case.},
it also increases the battery usage by \perc{200}\todo[author=Kasper]{Since there was some confusing regarding the percentage increase of accuracy, we better double check the battery usage.}.
