...
Smart home \emph{automation} is one of the big selling points of smart homes.
...
into the following three levels. Each level represents a degree of home automation.
...
The levels vary in the way users configure and interact with the systems. 
...
the state of devices in the system or the location of the user.

A wearable could be integrated as a type of sensor, 
for example to measure body temperature or CO\textsubscript{2} index.
The information can be included in rules that involve data about the user. 
...
but could also be a smartphone application. 
These types of systems are not combined with data from wearables, 
as they typically require a screen to select which device to control.
...
that replace manual care with an automated system \cite{nehmer2006living}. 
These systems gather environment and user data, 
to determine the user's future actions or intentions. 
The autonomous systems are designed to do what the users want, 
without the users' input or interaction. 
These systems apply methods and theories of statistics and machine learning, 
...
in order to ensure a better working environment.
...
Based on that data, wearables can be used to automate a lot of processes, 
...
The user defined rule, the interactive and the autonomous systems are all dependent on rules, 
